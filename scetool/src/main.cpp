...
//Emscripten
extern "C" {
	//! WARNING ! ALWAYS run init before attempting to process - the architecture of this program has been with antithetical to being long-running
	...
	int decrypt_self(char *in_file, char *out_file)
	{
		if(in_file == nullptr) EM_ASM(throw "in_file is null!");
		if(out_file == nullptr) EM_ASM(throw "out_file is null!");
		if(init()) EM_ASM(throw "Init Failed");

		return frontend_decrypt(in_file, out_file);
	}

	int encrypt_elf(char *in_file, char *out_file)
	{
		if(in_file == nullptr) EM_ASM(throw "in_file is null!");
		if(out_file == nullptr) EM_ASM(throw "out_file is null!");
		if(init()) EM_ASM(throw "Init Failed");
		...
		//tables.cpp
		char file_type_buffer[] = "SELF";
		char program_type_buffer[] = "APP";
		char key_rev_buffer[] = "1";
		char auth_id_buffer[] = "0x1010000001000003";
		char vendor_id_buffer[] = "0x01000002";
		char application_version_buffer[] = "1.00";

		_file_type = file_type_buffer;
		_program_type = program_type_buffer;
		_key_rev = key_rev_buffer;
		_auth_id = auth_id_buffer;
		_vendor_id = vendor_id_buffer;
		_app_version = application_version_buffer;

		return frontend_encrypt(in_file, out_file);
	}
}